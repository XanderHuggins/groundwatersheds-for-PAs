# Name: f2-UPR-modgrad.R
# Description: Plot UPR against human modification gradient at the scale of freshwater ecoregions
...
prot_pts$feow = rep(NA)
prot_pts$feow = raster::extract(x = raster(feow), y = prot_pts)

# keep only protected areas with identified FEOW + unprotected groundwatershed areas
prot_pts = prot_pts |> as.data.frame() |> 
  filter(unprot_gwshed_area > 0 & !is.na(feow))
...
# now import FEOW shapefile with habitat type attribute, and merge with data frame
feow_hab = sf::read_sf("D:/Geodatabase/Ecological/Ecoregions/data/commondata/data0/feow_ecoregions.shp") |> as.data.frame() #|> dplyr::select(ECO_ID, MHT_TXT, MHT_NO)

feow_mht_lookup = data.frame(
  ECO_ID = unique(feow_hab$ECO_ID),
  mht_txt = rep(NA),
  mht_no = rep(NA)
)

for (i in 1:nrow(feow_mht_lookup)) {
  temp_df = feow_hab |> filter(ECO_ID == feow_mht_lookup$ECO_ID[i])
  feow_mht_lookup$mht_txt[i] = temp_df$MHT_TXT[1]
  feow_mht_lookup$mht_no[i] = temp_df$MHT_NO[1]
}

stat_df = merge(x = stat_df, y = feow_mht_lookup, by.x = "feow", by.y = "ECO_ID", all.x = T)
...
# ecoregions to highlight
highlight_a = stat_df |> dplyr::filter(feow == 121) 
highlight_b = stat_df |> dplyr::filter(feow == 803)
highlight_c = stat_df |> dplyr::filter(feow == 165 | feow == 169)
highlight_d = stat_df |> dplyr::filter(feow == 404)

stat_df$mht_no = as.factor(stat_df$mht_no)
...
ggplot(data = as.data.frame(stat_df), aes(x = UPR, y = modgrad_mean, 
                                          fill = colorId)) +
  ...
  geom_point(data = highlight_a, aes(x = UPR, y = modgrad_mean),
             shape = 21, size = 5, fill = "#1C9F91", alpha = 1) +
  geom_point(data = highlight_b, aes(x = UPR, y = modgrad_mean),
             shape = 21, size = 5, fill = "#1C9F91", alpha = 1) +
  geom_point(data = highlight_c, aes(x = UPR, y = modgrad_mean),
             shape = 21, size = 5, fill = "#1C9F91", alpha = 1) +
  geom_point(data = highlight_d, aes(x = UPR, y = modgrad_mean),
             shape = 21, size = 5, fill = "#1C9F91", alpha = 1) +
  # scale_fill_gradientn(colors = c(scico(n = 100, palette = "romaO", direction = -1)),
  #                      limits = c(0.03, 0.2), oob = scales::squish) +
...
# global map
feow_hab = sf::read_sf("D:/Geodatabase/Ecological/Ecoregions/data/commondata/data0/feow_robin_vector_crs_fix.sqlite")
feow_hab = merge(x = feow_hab, y = stat_df, by.x = "eco_id", by.y = "feow")
feow_hab <- feow_hab %>% filter(!st_is_empty(.))

feow_tmap <- tm_shape(rnaturalearth::ne_countries(), projection = "+proj=robin") +
...
tmap_save(feow_tmap, file.path(plot_sv, "feow_upr-modgrad.png"), dpi = 400, units = "in")