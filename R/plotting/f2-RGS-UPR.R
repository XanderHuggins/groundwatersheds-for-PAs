...
for (i in 1:(nrow(line_df)-1)) {
  ...
  line_df$p90[i] <- quantile(x = temp_df$UPR, probs = 0.75, na.rm = T)
  line_df$p50[i] <- quantile(x = temp_df$UPR, probs = 0.5, na.rm = T)
  line_df$p10[i] <- quantile(x = temp_df$UPR, probs = 0.25, na.rm = T)
...
p_a11 <- ggplot(s_df, aes(x = log10(RGS), y = UPR)) +
  ...
  ylab('') + xlab('')+
  coord_cartesian(ylim = c(0,1), xlim = c(0, 1.2), expand = 0, clip = 'off')
p_a11
