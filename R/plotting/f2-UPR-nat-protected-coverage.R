...
# Description: Plot relationship between national protected area coverage and groundwatershed UPR.
...
# plot
p_a11 = ggplot(p_df, aes(x = AichiClass, y = UPR)) +
  geom_boxplot(data = p_df, aes(x = AichiClass, y = UPR), width = 0.35, fill = 'grey',
               outlier.alpha = 0, position= position_nudge(y=0)) +
  scale_x_discrete(labels = c('<1%', '1%-5%', '5%-10%', '10%-17%', '17-30%', '>30%')) +
  my_theme + theme(axis.text=element_text(colour="black"),
                   plot.margin = unit(c(2,5,2,2), "mm")) +
  ylab('') + xlab('')+
  coord_cartesian(ylim = c(0, 1), expand = 0, clip = 'on')
p_a11
