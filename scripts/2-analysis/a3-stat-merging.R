# Name: a3-stat-merging.R
# Description: Merge regional summary data into single data set
...
df2  = df2 |> group_by(CPA_ID) |> summarize(gde_area = sum(gde_area, na.rm = T)) # GDE area
...
df7  = df7 |> group_by(CPA_ID) |> summarize(aridity_xArea = sum(aridity_xArea, na.rm = T))
df8  = df8 |> group_by(CPA_ID) |> summarize(modgrad_xArea = sum(modgrad_xArea, na.rm = T))
...
# keep only protected areas with area > 1km2 (consistent with base resolution + filters out fragments not 
# disaggregated into larger contiguous protected area)
keep_df = c_df |> 
  filter(gde_area > 0) |> 
  dplyr::select(!c('aridity_xArea', 'modgrad_xArea'))

readr::write_csv(keep_df, file = file.path(dat_loc, "report_STATS.csv"))

# merge this with a shapefile of protected areas
pa_shp = terra::vect(file.path(wdpa_wd, "contiguous_protected_areas_FID.sqlite"))

keep_df = keep_df |> 
  dplyr::select('CPA_ID', 'RGS', 'UGR', 'modgrad')
pa_shp = merge(x = pa_shp, y = keep_df, by.x = "fid", by.y = "CPA_ID")
pa_shp = pa_shp[pa_shp$RGS >= 0,]

writeVector(x = pa_shp, 
            filename = file.path(dat_loc, "groundwatersheds.sqlite"),
