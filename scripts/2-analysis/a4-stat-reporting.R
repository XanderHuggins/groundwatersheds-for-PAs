...
s_df = readr::read_csv(file.path(dat_loc, "report_STATS.csv"))
nrow(s_df) #nrow = 32,490

sum(s_df$gwshed_area, na.rm = T)/1e12 # 23.04226 -> 21.96611 million km2 - area of groundwatersheds of protected areas considered
sum(s_df$area, na.rm = T)/1e12 # 12.55276 million km2 - area of protected areas considered
21.96611/12.55276 # 1.836094 -> 1.749903

# calculate median and IQR of RGS:
median(s_df$RGS) ## > 1.39112 -> 1.456501
quantile(s_df$RGS, 0.25) # 1.146544 -> 1.167467
quantile(s_df$RGS, 0.75) # 1.815177 -> 1.937667
...
1 - sum(s_df$unprot_gwshed_area, na.rm = T)/sum(s_df$gwshed_area, na.rm = T) # 54.4%
...
s_df = readr::read_csv(file.path(dat_loc, "report_STATS.csv"))
s_df = s_df |> filter(RGS >= 0 & UGR >= 0)
median(s_df$UGR) # 0.5183657 -> 0.5237834
quantile(s_df$UGR, 0.25) # 0.1665859  -> 0.1666321
quantile(s_df$UGR, 0.75) # 0.7963404 -> 0.7990331
# quantile(s_df$UGR, 0.49) # 0.5049899
